...
plot(1:nrow(data), data$Voltage, xaxt="n", type="l", ylab="Voltage", xlab="datetime")
axis(side=1, at=c(0,match(2,as.numeric(format(strptime(data$Date, format="%d/%m/%Y"), "%d"))), nrow(data)), labels = c("Thu","Fri","Sat"))
...
axis(side=1, at=c(0,match(2,as.numeric(format(strptime(data$Date, format="%d/%m/%Y"), "%d"))), nrow(data)), labels = c("Thu","Fri","Sat"))
legend("topright", lwd=rep(1,3), bty="n", col = palette, legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
plot(1:nrow(data), data$Global_reactive_power, xaxt="n", type="l", ylab="Global_reactive_power", xlab="datetime")
axis(side=1, at=c(0,match(2,as.numeric(format(strptime(data$Date, format="%d/%m/%Y"), "%d"))), nrow(data)), labels = c("Thu","Fri","Sat"))
dev.off()